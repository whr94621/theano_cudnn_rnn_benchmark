...
from dl4mt_layers.utils import init_tparams, itemlist, concatenate
import numpy as np
...
params = param_init_gru(params=params, nin=input_size, dim=hidden_size, prefix='gru')
params = param_init_gru(params=params, nin=input_size, dim=hidden_size, prefix='gru_r')

tparams = init_tparams(params=params)

proj = gru_layer(tparams=tparams, state_below=X, prefix='gru')
projr = gru_layer(tparams=tparams, state_below=X[::-1], prefix='gru_r')

outp = concatenate([proj[0], projr[0][::-1]], axis=proj[0].ndim-1)
cost = T.mean((Y - outp) ** 2)
grads = T.grad(cost, itemlist(tparams=tparams))


print("Non-CuDNN biidirectional GRU")
